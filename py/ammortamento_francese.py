...
def ammortamento_francese(capitale: float, i: float, n: int) -> pd.DataFrame:
    ...
    print("----- Debug -----")
    print("-----------------")
    print(f"a figurato n al tasso i: {round(a)}")
    print(f"rata costante: {round(rata_costante, 2)}")
    print("-----------------")
...
def gestisci_grafico(df: pd.DataFrame):
    """
    ...
    """
    # 1. Creazione tela con dimensioni standardizzate
    fig, ax = plt.subplots(figsize=(12, 7))
    
    # 2. Generazione del Bar Plot Impilato
    bars_cap = ax.bar(df['Periodo'], df['Quota Capitale (€)'], color='#2C5282', label="Quota Capitale", width=0.8)
    bars_int = ax.bar(df['Periodo'], df['Quota Interessi (€)'], bottom=df['Quota Capitale (€)'], color='#74A57F', label="Quota Interessi", width=0.8)
    
    # 3. Formattazione Assi (Font 13)
    ax.set_xlabel("Numero Rata", fontsize=13)
    ax.set_ylabel("Valore della Rata [€]", fontsize=13)
    ax.grid(axis='y', linestyle=':', color='gray', alpha=0.4)
    
    # 4. Titolo Globale
    fig.suptitle("Piano di Ammortamento a Rate Costanti (Francese)", fontsize=18, fontweight='bold')

    # 5. Legenda Centralizzata in Basso (y=0.07)
    ...
    # 6. Testo Note Metodologiche (y=0.01)
    testo_specifiche = (
        "Note Metodologiche: Evoluzione delle quote capitale e interessi nel tempo.\n"
        "Tasso fisso | Capitalizzazione composta | Rata costante."
    )
    ...
    # 7. Margine inferiore riservato aumentato al 22% (bottom=0.22)
    plt.tight_layout(rect=[0, 0.22, 1, 0.90])

    # 8. Logica di esportazione
...
if __name__ == "__main__":
    ...
    cap = acquisisci_input_numerico("Inserisci il capitale da richiedere a debito (in €): ", float)
    ...
    if mostra_grafico in ['s', 'si', 'y', 'yes']:
        gestisci_grafico(df_ammortamento)